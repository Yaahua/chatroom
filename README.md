...
### 💬 丰富的消息交互
- **@ 提及与通知**：输入 `@` 立即弹出智能面板，支持模糊匹配在线用户与 AI 助手，键盘快捷导航。被 @ 时消息气泡左侧显示橙色标记，标题栏出现 `@N` 角标，文本中的 `@昵称` 同步高亮。
...
- **系统消息优化**：区分首次连接与重连，避免重连时刷屏；系统提示采用轻量居中分割线样式，不干扰消息流阅读。
...
| 状态管理 | React Hooks（自定义 `useMqtt`、`useAI`、`useSound`） |
| 通信协议 | MQTT over WebSocket（基于 EMQX Cloud） |
| AI 引擎 | DeepSeek API（流式 SSE，支持 Vision 多模态） |
| 样式方案 | 原生 CSS3（CSS Variables、动态视口单位 `dvh`） |
...
│   │   ├── App.tsx               # 核心容器与状态分发
│   │   ├── ChatHeader.tsx        # 顶部导航、在线人数、@ 角标
│   │   ├── MessageList.tsx       # 消息流渲染、长按交互、@ 高亮
